...
public class HospitalApiController {
    ...
    // 根据医院名称返回医院列表
    @GetMapping("/name")
    public List<Hospital> findHospitalByHospitalName(@RequestParam("hospitalName") @NotBlank String hospitalName) {
        return hospitalService.findHospitalByHospitalName(hospitalName);
    }

    // 根据医院编号返回医院
    @GetMapping("/code/{hospitalCode}")
    public Hospital findHospitalByHospitalCode(@PathVariable("hospitalCode") @NotBlank String hospitalCode) {
        return hospitalService.findHospitalByHospitalCode(hospitalCode);
    }

    // 条件查询带分页
    @PostMapping("/page")
    public IPage<Hospital> findHospitalPage(@RequestBody @NotNull HospitalQueryVO hospitalQueryVO,
